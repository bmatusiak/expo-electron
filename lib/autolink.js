...
function makePreload(mods, outPath) {
    ...
    for (const m of mods) {
        ...
        lines.push(`  const prodPathBase = path.join(process.resourcesPath || process.cwd(), 'native', '${m.name}');`);
        let prodRequire = 'null';
        if (m.expoBlock && m.expoBlock.entry) {
            const base = path.basename(m.expoBlock.entry);
            prodRequire = `path.join(prodPathBase, '${base}')`;
        } else if (m.entry === 'electron') {
            prodRequire = `path.join(prodPathBase, 'index.js')`;
        }
        if (prodRequire === 'null') lines.push(`  const prodPath = null;`);
        else lines.push(`  const prodPath = ${prodRequire};`);
...
function makeResources(mods, projectRoot) {
    ...
    for (const m of mods) {
        ...
        if (fs.existsSync(electronRoot)) {
            // produce project-root-relative path using POSIX separators
            let rel = path.relative(projectRoot, electronRoot).replace(/\\/g, '/');
            if (!rel) rel = '.';
            resources.push({ from: rel, to: `native/${m.name}` });
