...
function makePreload(mods, outPath, projectRoot) {
  ...
  const _resBase = (process.resourcesPath || process.cwd());`);
    ...
    // Provide a backward-compatible alias `native` on the global so existing
    // code referencing `globalThis.native` continues to work. Prefer the
    // explicit `ElectronNative` name for clarity.
    lines.push("try { globalThis.ElectronNative = globalThis.ElectronNative || ElectronNative; if (!globalThis.native) globalThis.native = ElectronNative; } catch (e) {};");
